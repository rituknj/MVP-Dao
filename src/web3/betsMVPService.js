...
export const addRefLink = async (id, ref) => {
  const betMVPContract = await getContract(
    MVPBetsV2,
    envdev.REACT_APP_BET_BETSWAMP_V2
  );
  const data = await betMVPContract.methods
    .setRefLink(await getAccount(), id, ref)
    .send({
...
export const createEvent = async (
  ...
  var getData = await betMVPContract.methods
    .createEvent(
      num,
      sub_category,
      name,
...
export const placeBet = async (event_id, occured, amount) => {
  ...
  if (0 < Number(maxamount)) {
    ...
    if (approve.status == true) {
      getData = await betMVPContract.methods
        .placeBet(event_id, a, occured)
        .send({
          from: await getAccount(),
        });
      return getData;
...
export const validateEvent = async (event_id, occured) => {
  ...
  var getData = await betMVPContract.methods
    .validateEvent(event_id, occured)
    .send({
      from: await getAccount(),
    });
    return getData;
...
export const getValidationPoint = async () => {
  ...
  const validationPoint = await betMVPContract.methods
    .showValidationPoints(await getAccount())
    .call();
...
export const CreatorReward = async (id) => {
  ...
  const betsHistory = await betMVPContract.methods
    .getCreatorRewardOnEvent(await getAccount(), id)
    .call();
  return betsHistory;
};
...
export const UserEventHistory = async () => {
  ...
  const betsHistory = await betMVPContract.methods
    .getUserEventHistory(await getAccount())
    .call();
  return betsHistory;
};
...
export const BoostEvent = async (id) => {
  ...
  const betsHistory = await betMVPContract.methods
    .boostEvent("5000000000000000000", id)
...
export const getOccurrenceBetCount = async (eventID, occured) => {
  ...
  const occurrenceBetCount = await betMVPContract.methods
    .getOccurrenceBetCount(eventID, occured)
    .call();
...
export const getEventOccurrenceBetAmount = async (eventID, occured) => {
  ...
  const eventOccurrenceBetAmount = await betMVPContract.methods
    .getEventOccurrenceBetAmount(eventID, occured)
    .call();
...
export const revokevalidationpointsearning = async () => {
  const betMVPContract = await getContract(Points, envdev.REACT_AAP_POINTS);
  const earnedpoints = await betMVPContract.methods
...
export const totaltokenlocked = async () => {
  const betMVPContract = await getContract(Points, envdev.REACT_AAP_POINTS);
  const earnedpoints = await betMVPContract.methods
    .userCurrentlyLockedBETS(await getAccount())
    .call();
  return earnedpoints;
};
...
export const getusertotalwinnings = async () => {
  ...
  const earnedpoints = await betMVPContract.methods
    .getUserTotalWinnings(await getAccount())
    .call();
  return earnedpoints;
};
...
export const gettotaluserwageramount = async () => {
  ...
  const earnedpoints = await betMVPContract.methods
    .getTotalUserWagerAmount(await getAccount())
    .call();
  return earnedpoints;
};
...
export const bettorscounts = async (id, occured) => {
  ...
  let result = await betMVPContract.methods
    .getOccurrenceBetCount(id, occured)
    .call();
  return result;
};
...
export const bettorscountspercent = async (id, occured, bettcont) => {
  ...
  const bettes =
    ((await betMVPContract.methods.getOccurrenceBetCount(id, occured).call()) /
      bettcont) *
    100;
  return bettes;
};
...
export const AmountStackOnEventByaUser = async (id) => {
  ...
  const resutl = await betMVPContract.methods
    .getUserEventWager(id, await getAccount())
    .call();
  return resutl;
};
...
export const GetUserWonAmountOnEvent = async (id) => {
  ...
  const resutl = await betMVPContract.methods
    .getUserEventWon(id, await getAccount())
    .call();
  return resutl;
};
...
export const pendingpoint = async () => {
  const betMVPContract = await getContract(Points, envdev.REACT_AAP_POINTS);
  const resutl = await betMVPContract.methods
    .getUserPendingPoints(await getAccount())
    .call();
...
export const userBethistory = async (id, address) => {
  ...
  const resutl = await betMVPContract.methods
    .getUserBetHistory(await getAccount())
    .call();
  return resutl;
};
...
export const getvalidatorHistory = async () => {
  ...
  const resutl = await betMVPContract.methods
    .validatorHistory(await getAccount())
    .call();
  return resutl;
};
...
export const validatorsRewardOnEvnet = async (id) => {
  ...
  const resutl = await betMVPContract.methods
    .getValidatorRewardOnEvent(await getAccount(), id)
    .call();
  return resutl;
};
...
export const claimTestBets = async () => {
  const betMVPContract = await getContract(
    BETS_Faucet,
    envdev.REACT_APP_BET_FAUCET
  );
  const data = betMVPContract.methods
